namespace PowerTask
{
        ...
        INotifyPropertyChanged
    {
        ...
        private void PaintBackgroundLayer(CanvasDrawingSession drawingSession, List<VtNetCore.VirtualTerminal.Layout.LayoutRow> spans)
        {
            double lineY = 0;
        ...
        private void PaintTextLayer(CanvasDrawingSession drawingSession, List<VtNetCore.VirtualTerminal.Layout.LayoutRow> spans, CanvasTextFormat textFormat, bool showBlink)
        {
            var dipToDpiRatio = 10;//= Windows.Graphics.Display.DisplayInformation.GetForCurrentView().LogicalDpi / 96;
        ...
        private void PaintCursor(CanvasDrawingSession drawingSession, List<VtNetCore.VirtualTerminal.Layout.LayoutRow> spans, CanvasTextFormat textFormat, TextPosition cursorPosition, Color cursorColor)
        {
        ...
        public void CoreWindow_CharacterReceived(UIElement sender, CharacterReceivedRoutedEventArgs args)
        {
            //if (!Connected)
            //    return;

            var ch = char.ConvertFromUtf32((int)args.Character);
            switch (ch)
            {
                case "\b":
                case "\t":
        ...
        private void TerminalKeyDown(object sender, KeyRoutedEventArgs e)
        {
            ...
            var controlPressed = (InputKeyboardSource.GetKeyStateForCurrentThread(Windows.System.VirtualKey.Control).HasFlag(CoreVirtualKeyStates.Down));
            var shiftPressed = (InputKeyboardSource.GetKeyStateForCurrentThread(Windows.System.VirtualKey.Shift).HasFlag(CoreVirtualKeyStates.Down));
        ...
        {
            // TODO
            canvas.CharacterReceived += CoreWindow_CharacterReceived;
